...
void tmpInfo(String text,boolean jam) {
  static uint32_t pM;
  uint32_t cM = millis();
...
void animBuka(uint32_t x){
  display.drawFilledRect(0,0,63-x,6,0);
  display.drawFilledRect(64+x,0,128,6,0);
  display.drawLine(63-x,0,63-x,6);
  display.drawLine(64+x,0,64+x,6);
}

void animTutup(uint32_t x){
  display.drawFilledRect(0,0,0+x,6,0);
  display.drawFilledRect(128-x,0,128,6,0);
  display.drawLine(0+x,0,0+x,6);
  display.drawLine(128-x,0,128-x,6);
}
void infoIqomah()
{
...
void tmpWaktu(int imsaq, int shubuh, int shuruq, int dhuha, int dzuhur, int ashar, int magrib, int isya) {
  ...
  if(cM - pM > Kecepatan) {
   ...
   if (i==1){
    ...
    display.drawText(36, 0, bufferText);
    animBuka(x);
   }else if(i==2){
    jam=shubuh/100; menit=shubuh%100;
    sprintf(bufferText,"SHUBUH:%02d:%02d",jam,menit);
...
void tmpJamKecil(boolean tmp) {
 ...
 int detik = now.second();
  ...
  if(tmp)display.setFont(arab6x13);
  else   display.setFont(angka6x13);
  display.drawFilledRect(0,0,31,1,0);
  sprintf(bufferInfo,"%d%d    %d%d  ",jam/10,jam%10,menit/10,menit%10,detik/10,detik%10);
  display.drawText(0,1,bufferInfo); 
  if(detik%2==0){
   display.drawCircle(15, 4, 1);display.drawCircle(15, 11, 1);
  }
  else {
   display.drawFilledCircle(15, 4, 1);display.drawFilledCircle(15, 11, 1);
  }
}
...
void countIqomah(byte l_iqomah)
{
  ...
  for(int i=1;i<w_iqomah;i++)
  {
   ...
   display.drawText(((display.width()-display.textWidth(bufferInfo))/2)+1,4,bufferInfo);
   display.swapBuffers();
   delay(1000);
  }  
}
...
void setIqomah(char iqomah, int lama)
{
 ...
 switch(iqomah)
 {
  case 'd' : sprintf(bufferInfo,"Iqom dzuhur %d%dm",lama/10,lama%10);
             display.drawText(((display.width()-display.textWidth(bufferInfo))/2)+1,3,bufferInfo);
             break;
  case 'a' : sprintf(bufferInfo,"Iqom ashar %d%dm",lama/10,lama%10);
             display.drawText(((display.width()-display.textWidth(bufferInfo))/2)+1,3,bufferInfo);
             break;
  case 'm' : sprintf(bufferInfo,"Iqom magrib %d%dm",lama/10,lama%10);
             display.drawText(((display.width()-display.textWidth(bufferInfo))/2)+1,3,bufferInfo);
             break;
  case 'i' : sprintf(bufferInfo,"Iqom isya' %d%dm",lama/10,lama%10);
             display.drawText(((display.width()-display.textWidth(bufferInfo))/2)+1,3,bufferInfo);
             break;
  case 's' : sprintf(bufferInfo,"Iqom shubuh %d%dm",lama/10,lama%10);
             display.drawText(((display.width()-display.textWidth(bufferInfo))/2)+1,3,bufferInfo);
             break;
  }  
  display.swapBuffers();
