...
void animBuka(uint32_t x){
  display.drawFilledRect(30,0,87-x,6,0);
  display.drawFilledRect(87+x,0,144,6,0);
  display.drawLine(87-x,0,87-x,6);
  display.drawLine(87+x,0,87+x,6);
}

void animTutup(uint32_t x){
  display.drawFilledRect(30,0,30+x,6,0);
  display.drawFilledRect(144-x,0,144,6,0);
  display.drawLine(30+x,0,30+x,6);
  display.drawLine(144-x,0,144-x,6);
}
void infoIqomah()
{
...
void tmpWaktu(int imsaq, int shubuh, int shuruq, int dhuha, int dzuhur, int ashar, int magrib, int isya) {
  ...
  if(cM - pM > Kecepatan) {
   ...
   if (i==1){
    ...
    display.drawText(52, 0, bufferText);
    animBuka(x);
   }else if(i==2){
    jam=shubuh/100; menit=shubuh%100;
    sprintf(bufferText,"SHUBUH : %02d:%02d",jam,menit);
