...
void tmpJadwalSholat() {
  ...
  if((millis() - pM) > 4000) { 
   ...
   if (i==1){ 
    jam1=jadwal.imsak/100;  menit1=jadwal.imsak%100;
    jam2=jadwal.shubuh/100; menit2=jadwal.shubuh%100;
    sprintf(bufferText,"IMSAK %02d:%02d SHUBUH %02d:%02d",jam1,menit1,jam2,menit2);
    Disp.drawText(71, 0, bufferText);
   }else if (i==2){ 
    jam1=jadwal.shuruq/100; menit1=jadwal.shuruq%100;
    jam2=jadwal.dhuha/100;  menit2=jadwal.dhuha%100;
    sprintf(bufferText,"SHURUQ %02d:%02d DHUHA %02d:%02d",jam1,menit1,jam2,menit2);
    Disp.drawText(71, 0, bufferText);
   }else if (i==3){ 
    jam1=jadwal.dzuhur/100; menit1=jadwal.dzuhur%100;
    jam2=jadwal.ashar/100;  menit2=jadwal.ashar%100;
    sprintf(bufferText,"DZUHUR %02d:%02d ASHAR %02d:%02d",jam1,menit1,jam2,menit2);
    Disp.drawText(71, 0, bufferText);
   }else if(i==4){
    jam1=jadwal.magrib/100; menit1=jadwal.magrib%100;
    jam2=jadwal.isya/100;   menit2=jadwal.isya%100;
    sprintf(bufferText,"MAGRIB %02d:%02d ISYA' %02d:%02d",jam1,menit1,jam2,menit2);
    Disp.drawText(71, 0, bufferText);
   }
}
...
void tmpJam(){
  ...
  //logoA(112);
  //logoM(32);
  Disp.setFont(SevSeg2);
  sprintf(bufferJam,"%02d    %02d    %02d ",rJam,rMen,rDet);
  Disp.drawText(0,0,bufferJam); 
  if(rDet%2==0){
   Disp.drawCircle(21, 4, 1);Disp.drawCircle(21, 11, 1);
   Disp.drawCircle(45, 4, 1);Disp.drawCircle(45, 11, 1);
  }
  else {
   Disp.drawFilledCircle(21, 4, 1);Disp.drawFilledCircle(21, 11, 1);
   Disp.drawFilledCircle(45, 4, 1);Disp.drawFilledCircle(45, 11, 1);
  }
}
...
void tmpjam(){
  ...
  Disp.setFont(SmallCap4x6);
  sprintf(bufferJam,"%02d:%02d:%02d ",rJam,rMen,rDet);
  Disp.drawText(76,0,bufferJam); 
}


void Utama(){
  if(rHar==5 and rJam>=11 and rJam<12){
   sprintf(bufferText,"%s",jumat[wHar0]);
   tmpInfo(bufferText);
   tmpjam();
  }else if(tmputama==0){
...
void Iqomah() {
  ...
  Disp.setFont(ElektronMart6x8);
  Disp.drawText(51,4, "IQOMAH");
  sprintf(bufferIqom, "%02d:%02d", menitiqmh, detikiqmh);
  Disp.drawText(108,4, bufferIqom);
